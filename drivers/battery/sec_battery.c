	...
	if (battery->siop_level == 3) {
		...
		if (battery->cable_type == POWER_SUPPLY_TYPE_WIRELESS ||
			...
			*charging_current = battery->pdata->charging_current[
			...
			battery->input_current_step_up = true;
		}
...
static int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		if (battery->hv_overheating) {
			input_current = (input_current > battery->pdata->hv_overheat_input_current_limit) ?
				battery->pdata->hv_overheat_input_current_limit : input_current;
		}
	}
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_WIRELESS ||
		...
		if (battery->input_current_step_up == true) {
			if (battery->wired_input_current != input_current) {
				battery->wired_input_current = input_current;
				wake_lock(&battery->current_step_wake_lock);
				cancel_delayed_work(&battery->current_step_work);
				queue_delayed_work(battery->monitor_wqueue,
					&battery->current_step_work, 0);
			}
		} else {
...
static int sec_bat_adc_ap_read(struct sec_battery_info *battery, int channel)
{
	...
	case SEC_BAT_ADC_CHANNEL_TEMP:
		rc = qpnp_vadc_read(adc_client, battery->pdata->p_mux_channel, &results);
		if (rc) {
	...
	case SEC_BAT_ADC_CHANNEL_BAT_CHECK:
		if (!battery->pdata->batt_channel) {
			...
			pr_info("BAT_ID physical= %lld, raw = 0x%x\n", results.physical, results.adc_code);
			data = results.physical;
		} else {
			rc = qpnp_vadc_read(adc_client, battery->pdata->batt_channel, &results);
			if (rc) {
				pr_err("%s: Unable to read batt temperature rc=%d\n",
				       __func__, rc);
				return 0;
...
#if defined(CONFIG_SEC_ON7XLTE_CHN)
...
static bool sec_bat_check_vf_gpio(struct sec_battery_info *battery)
{
	union power_supply_propval value;
...
static bool sec_bat_check(struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_CHECK_INT:
		if(battery->cable_type == POWER_SUPPLY_TYPE_BATTERY)
			...
			ret = sec_bat_check_vf_gpio(battery);
		break;
...
temp_by_adc_goto:
#if defined(CONFIG_USE_TEMP_COMPENSATION)
	/* Temp compensation */
	if (channel == SEC_BAT_ADC_CHANNEL_TEMP){
		if(battery->cable_type == POWER_SUPPLY_TYPE_HV_MAINS){
			temp = temp-35;
	...
	struct sec_battery_info *battery)
{
	...
	/* HV charging , Prevent heat up and Scald */
	if (battery->pdata->hv_overheat_check &&
			battery->siop_level >= 100 && battery->pdata->chg_temp_check &&
			((battery->cable_type == POWER_SUPPLY_TYPE_HV_MAINS) ||
			 (battery->cable_type == POWER_SUPPLY_TYPE_HV_ERR))) {
		if (battery->temperature > battery->pdata->hv_overheat_batt_temp_block &&
			battery->chg_temp > battery->pdata->hv_overheat_chg_temp_block) {
			if (!battery->hv_overheating) {
				battery->hv_overheating = true;
				dev_info(battery->dev,"%s: HV current is reduced by hv_overheating(%d).(batt_temp=%d,chg_temp=%d)\n",
						__func__, battery->hv_overheating, battery->temperature, battery->chg_temp);
				sec_bat_set_charging_current(battery);
			}
			return;
		} else if (battery->hv_overheating) {
			if (battery->temperature < battery->pdata->hv_overheat_batt_temp_recov) {
				battery->hv_overheating = false;
				dev_info(battery->dev,"%s: HV current is recovered by hv_overheating(%d).(batt_temp=%d,chg_temp=%d)\n",
						__func__, battery->hv_overheating, battery->temperature, battery->chg_temp);
				sec_bat_set_charging_current(battery);
			}
			return;
	...
	if (battery->siop_level >= 100 && battery->pdata->chg_temp_check &&
		...
		if ((battery->chg_limit == SEC_BATTERY_CHG_TEMP_NONE) &&
			...
			if(battery->pdata->chg_charging_limit_current){
			...
			if(battery->pdata->chg_high_temp_chg_current){
				if (battery->charging_current > battery->pdata->chg_high_temp_chg_current) {
					value.intval = battery->pdata->chg_high_temp_chg_current;
					psy_do_property(battery->pdata->charger_name, set,
							POWER_SUPPLY_PROP_CURRENT_NOW, value);
			...
			if(battery->pdata->chg_charging_limit_current_2nd){
			...
			if(battery->pdata->chg_high_temp_chg_current_2nd){
				if (battery->charging_current > battery->pdata->chg_high_temp_chg_current_2nd) {
					value.intval = battery->pdata->chg_high_temp_chg_current_2nd;
					psy_do_property(battery->pdata->charger_name, set,
							POWER_SUPPLY_PROP_CURRENT_NOW, value);
					battery->charging_current = value.intval;
				}
...
static void sec_bat_current_step_work(struct work_struct *work)
{
	struct sec_battery_info *battery = container_of(work,
				struct sec_battery_info, current_step_work.work);
	union power_supply_propval value;
	int ret1, ret2;
	int current_value = 0;
	int pre_wired_input_current = battery->wired_input_current;
	pr_info("%s Start\n", __func__);

	while (pre_wired_input_current == battery->wired_input_current &&
			current_value < battery->wired_input_current) {

		current_value += 100;
		current_value = (current_value > battery->wired_input_current) ?
								battery->wired_input_current : current_value;

		value.intval = current_value;
		psy_do_property(battery->pdata->charger_name, set,
			POWER_SUPPLY_PROP_CURRENT_MAX, value);
		...
		pr_info("%s : set current %dmA. (%d %d) [%dmA]\n", __func__, current_value, ret1, ret2, battery->wired_input_current);
		if (ret1 != ret2) // current_max already change.
			break;
	}

	pr_info("%s End\n", __func__);
	wake_unlock(&battery->current_step_wake_lock);
}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case CHECK_SLAVE_CHG:
		psy_do_property(battery->pdata->charger_name, get,
			POWER_SUPPLY_PROP_READ_SLAVE_REG, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			value.intval);
	...
	case MODE:
			psy_do_property(battery->pdata->charger_name, get,
				POWER_SUPPLY_PROP_MULTI_CHARGER_MODE, value);
			i += scnprintf(buf + i, PAGE_SIZE - i, "%s\n",
				value.strval);
			break;
#if defined(CONFIG_SAMSUNG_BATTERY_ENG_TEST)
	...
	case CISD_DATA:
		{
			...
			i+= scnprintf(buf + i, PAGE_SIZE - i, "%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
	...
	case CISD_DATA_JSON:
		{
			...
			pcisd->data[CISD_DATA_CAP_ONCE] = -1;
			pcisd->data[CISD_DATA_CAP_PER_TIME] = -1;
		}
		break;
	...
	case CISD_DATA:
		{
			...
			if (sscanf(buf, "%10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d, %10d\n",
					...
					&temp_data[30], &temp_data[31], &temp_data[32]) == CISD_DATA_MAX) {
				struct cisd *pcisd = &battery->cisd;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_USB_CONFIGURE:
...
#if defined(CONFIG_SEC_FACTORY) && defined(CONFIG_FUELGAUGE_S2MU005)
		current_cable_type = POWER_SUPPLY_TYPE_BATTERY;
		break;
#endif
		...
		struct sec_battery_info *battery)
{
	...
	if (ret) {
		...
		pdata->cisd_cap_high_thr = -1;
	}
	ret = of_property_read_u32(np, "battery,cisd_cap_low_thr",
		&pdata->cisd_cap_low_thr);
	if (ret) {
		pr_info("%s : cisd_cap_low_thr is Empty\n", __func__);
		pdata->cisd_cap_low_thr = -1;
	}
	ret = of_property_read_u32(np, "battery,cisd_cap_limit",
		&pdata->cisd_cap_limit);
	if (ret) {
		pr_info("%s : cisd_cap_limit is Empty\n", __func__);
		pdata->cisd_cap_limit = -1;
	}
	ret = of_property_read_u32(np, "battery,full_chg_current_margin",
		&temp);
	pdata->full_chg_current_margin = (int)temp;
	if (ret) {
		pr_info("%s : full_chg_current_margin is Empty, Defualt value 0\n", __func__);
		pdata->full_chg_current_margin = 0;
	}
#endif
	...
	if (pdata->thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC) {
		...
		ret = of_property_read_u32(np, "battery,mux_channel",
					   &pdata->p_mux_channel);
		if (ret) {
			pr_info("%s : Mux Channel is Empty.\n", __func__);
			pdata->p_mux_channel = 2;
		}

		switch(pdata->p_mux_channel) {
		case 1 :
			pdata->p_mux_channel = P_MUX1_1_1;
			break;
		case 2:
			pdata->p_mux_channel = P_MUX2_1_1;
			break;
		case 3:
			pdata->p_mux_channel = P_MUX3_1_1;
			break;
		case 4:
			pdata->p_mux_channel = P_MUX4_1_1;
			break;
		default:
			pdata->p_mux_channel = P_MUX2_1_1;
			break;
	...
	if (pdata->chg_temp_check) {
		...
		if (ret){
			...
			pdata->chg_charging_limit_current = 0;
		}

		ret = of_property_read_u32(np, "battery,chg_high_temp_chg_current",
					   &pdata->chg_high_temp_chg_current);
		if (ret){
			pr_info("%s : chg_high_temp_chg_current is Empty\n", __func__);
			pdata->chg_high_temp_chg_current = 0;
		}

		ret = of_property_read_u32(np, "battery,chg_high_temp_chg_current_2nd",
					   &pdata->chg_high_temp_chg_current_2nd);
		if (ret){
			pr_info("%s : chg_high_temp_chg_current_2nd is Empty\n", __func__);
			pdata->chg_high_temp_chg_current_2nd = 0;
		}
		...
		if (ret){
			...
			pdata->chg_charging_limit_current_2nd = 0;
		}
	...
	if (pdata->battery_check_type == SEC_BATTERY_CHECK_ADC) {
		ret = of_property_read_u32(np, "battery,batt_channel",
					   &pdata->batt_channel);
		if (ret) {
			pr_info("%s : Mux Channel is Empty.\n", __func__);
			pdata->batt_channel = 0;
		} else {
			switch(pdata->batt_channel) {
			case 1 :
				pdata->batt_channel = P_MUX1_1_1;
				break;
			case 2:
				pdata->batt_channel = P_MUX2_1_1;
				break;
			case 3:
				pdata->batt_channel = P_MUX3_1_1;
				break;
			case 4:
				pdata->batt_channel = P_MUX4_1_1;
				break;
			default:
				pdata->batt_channel = 0;
				break;
	...
	if (pdata->bat_irq_gpio < 0)
		pdata->bat_irq_gpio = 0;
	...
	ret = of_property_read_u32(np, "battery,swelling_float_voltage_margin",
		(unsigned int *)&pdata->swelling_float_voltage_margin);
	if (ret) {
		pr_info("%s: swelling float voltage margin is Empty, Default value 0mV \n", __func__);
		pdata->swelling_float_voltage_margin = 0;
	}
	...
	ret = of_property_read_u32(np, "battery,hv_overheat_check",
		&pdata->hv_overheat_check);
	if (ret)
		pr_info("%s : battery,hv_overheat_check is Empty\n", __func__);

	if (pdata->hv_overheat_check) {
		pr_info("%s : Enable hv_overheat_check. \n", __func__);
		
		ret = of_property_read_u32(np, "battery,hv_overheat_batt_temp_block",
					   &temp);
		pdata->hv_overheat_batt_temp_block = (int)temp;
		if (ret) {
			pr_info("%s: hv_overheat_batt_temp_block is Empty\n", __func__);
			pdata->hv_overheat_batt_temp_block = 1000;
		}

		ret = of_property_read_u32(np, "battery,hv_overheat_chg_temp_block",
					   &temp);
		pdata->hv_overheat_chg_temp_block = (int)temp;
		if (ret) {
			pr_info("%s: hv_overheat_chg_temp_block is Empty\n", __func__);
			pdata->hv_overheat_chg_temp_block = 1000;
		}

		ret = of_property_read_u32(np, "battery,hv_overheat_batt_temp_recov",
					   &temp);
		pdata->hv_overheat_batt_temp_recov = (int)temp;
		if (ret) {
			pr_info("%s: hv_overheat_batt_temp_recov is Empty\n", __func__);
			pdata->hv_overheat_batt_temp_recov = 1000;
		}

		ret = of_property_read_u32(np, "battery,hv_overheat_input_current_limit", 
						&pdata->hv_overheat_input_current_limit);
		if (ret) {
			pr_info("%s: hv_overheat_input_current_limit is Empty, Defualt value 1650mA \n", __func__);
			pdata->hv_overheat_input_current_limit = 1650;
		}
...
#if defined(CONFIG_SEC_MPP_SHARE)
		np = of_find_node_by_name(NULL, "mux_sel");
		pdata->mpp2_mux_sel = of_get_named_gpio(np, "mux_sel,mpp2_mux_sel", 0);
		pr_info("%s : mpp2_mux_sel %d \n", __func__,pdata->mpp2_mux_sel);
		if (pdata->mpp2_mux_sel < 0)
			pdata->mpp2_mux_sel = 0;

		pdata->mpp4_mux_sel = of_get_named_gpio(np, "mux_sel,mpp4_mux_sel", 0);
		pr_info("%s : mpp4_mux_sel %d \n", __func__,pdata->mpp4_mux_sel);
		if (pdata->mpp4_mux_sel < 0)
			pdata->mpp4_mux_sel = 0;
#endif
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&battery->afc_wake_lock, WAKE_LOCK_SUSPEND,
		       "sec-battery-afc");
	wake_lock_init(&battery->current_step_wake_lock, WAKE_LOCK_SUSPEND,
		       "sec-battery-current-step");
...
err_wake_lock:
	...
	wake_lock_destroy(&battery->current_step_wake_lock);
